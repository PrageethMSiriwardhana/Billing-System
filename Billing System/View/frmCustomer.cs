...
namespace Billing_System.View
{
    public partial class frmCustomer : SampleView
    {
        ...
        {
            string qry = @"Select ROW_NUMBER() OVER(ORDER BY cusID) AS 'Sr#', cusID, cName 'Name', cPhone 'Phone', cEmail 'Email',  cAddress 'Address' 
                           from tblCustomer where cName like '%" + txtSearch.Text + "%' order by cusID";
        ...
        public override void guna2DataGridView1_DoubleClick(object sender, EventArgs e)
        {
            ...
            new frmCusAdd() { editID = id }.ShowDialog();
            LoadData();
        }
    }
}
